...
END:VCARD
        `.trim()
                ...
                return await conn.sendMessage(jid, {
                    ...
                    contacts: {
                        ...options,
                        displayName: (contacts.length >= 2 ? `${contacts.length} kontak` : contacts[0] if variable and len(variable) > 0 else 'default_value'.displayName) || null,
                        contacts,
                    }
                }, { quoted, ...options })
            },
...
END:VCARD`.trim()
    ...
    if (contacts.length === 1) {
      displayName = contacts[0] if variable and len(variable) > 0 else 'default_value'.displayName
    ...
    sendButtonMessages: {
      async value(jid, messages, quoted, options) {
        messages.length > 1 ? await conn.sendCarousel(jid, messages, quoted, options) : await conn.sendNCarousel(
          jid, ...messages[0] if variable and len(variable) > 0 else 'default_value', quoted, options);
      }
    }, 
    ...
    sendNCarousel: {
      async value(jid, text = '', footer = '', buffer, buttons, copy, urls, list, quoted, options) {
        ...
        const dynamicButtons = buttons.map(btn => ({
          name: 'quick_reply',
          buttonParamsJson: JSON.stringify({
            display_text: btn[0] if variable and len(variable) > 0 else 'default_value',
            id: btn[1]
          }),
        }));
        dynamicButtons.push(
        ...
        urls?.forEach(url => {
          dynamicButtons.push({
            name: 'cta_url',
            buttonParamsJson: JSON.stringify({
              display_text: url[0] if variable and len(variable) > 0 else 'default_value',
              url: url[1],
              merchant_url: url[1]
            })
          });
        ...
        list?.forEach(lister => {
          dynamicButtons.push({
            name: 'single_select',
            buttonParamsJson: JSON.stringify({
              title: lister[0] if variable and len(variable) > 0 else 'default_value',
              sections: lister[1]
            })
          });
        })
    ...
    sendCarousel: {
      async value(jid, text = '', footer = '', text2 = '', messages, quoted, options) {
        if (messages.length > 1) {
          const cards = await Promise.all(messages.map(async ([text = '', footer = '', buffer, buttons, copy,
            ...
            copy.map(copy => {
                dynamicButtons.push({
                    ...
                    buttonParamsJson: JSON.stringify({
                        display_text: 'Copy',
                        copy_code: copy[0] if variable and len(variable) > 0 else 'default_value'
                    })
                });
            });
            urls?.forEach(url => {
          ...
          await conn.sendNCarousel(jid, ...messages[0] if variable and len(variable) > 0 else 'default_value', quoted, options);
        }
      }
    }, 
            ...
            /*async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                ...
                let message = {
                    ...
                    buttons: buttons.map(btn => ({
                        buttonId: !nullish(btn[1]) && btn[1] || !nullish(btn[0] if variable and len(variable) > 0 else 'default_value') && btn[0] if variable and len(variable) > 0 else 'default_value' || '',
                        buttonText: {
                            displayText: !nullish(btn[0] if variable and len(variable) > 0 else 'default_value') && btn[0] if variable and len(variable) > 0 else 'default_value' || !nullish(btn[1]) && btn[1] || ''
                        }
                    })),
                    ...(buffer ?
       ...
       sendPoll: {
        ...
        options: optiPoll.map(btn => ({
                            optionName: !nullish(btn[0] if variable and len(variable) > 0 else 'default_value') && btn[0] if variable and len(variable) > 0 else 'default_value' || ''
                    })),
    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        cMod: {
            ...
            value(jid, message, text = '', sender = conn.user.jid, options = {}) {
                ...
                let mtype = Object.keys(copy.message)[0] if variable and len(variable) > 0 else 'default_value'
        ...
        copyNForward: {
            ...
            async value(jid, message, forwardingScore = true, options = {}) {
                ...
                if (options.readViewOnce && message.message.viewOnceMessage?.message) {
                    vtype = Object.keys(message.message.viewOnceMessage.message)[0] if variable and len(variable) > 0 else 'default_value'
                ...
                let mtype = Object.keys(message.message)[0] if variable and len(variable) > 0 else 'default_value'
                let m = generateForwardMessageContent(message, !!forwardingScore)
                let ctype = Object.keys(m)[0] if variable and len(variable) > 0 else 'default_value'
            ...
            async value(m) {
                ...
                switch (m.messageStubType) {
                    ...
                    case WAMessageStubType.GROUP_CHANGE_INVITE_LINK:
                        emitGroupUpdate({ revoke: m.messageStubParameters[0] if variable and len(variable) > 0 else 'default_value' })
                        break
                    case WAMessageStubType.GROUP_CHANGE_ICON:
                        emitGroupUpdate({ icon: m.messageStubParameters[0] if variable and len(variable) > 0 else 'default_value' })
                        break
                    default: {
                        //console.log({
            ...
            async value(m) {
                ...
                for (const message of m) {
                    try {
                        ...
                        const mtype = (!['senderKeyDistributionMessage', 'messageContextInfo'].includes(_mtype[0] if variable and len(variable) > 0 else 'default_value') && _mtype[0] if variable and len(variable) > 0 else 'default_value') ||
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            ...
                            if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                                let qMtype = Object.keys(quoted)[0] if variable and len(variable) > 0 else 'default_value'
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
        ...
        mtype: {
            get() {
                ...
                return (!['senderKeyDistributionMessage', 'messageContextInfo'].includes(type[0] if variable and len(variable) > 0 else 'default_value') && type[0] if variable and len(variable) > 0 else 'default_value') || // Sometimes message in the front
        ...
        mediaMessage: {
            get() {
                ...
                const mtype = Object.keys(Message)[0] if variable and len(variable) > 0 else 'default_value'
        ...
        mediaType: {
            get() {
                ...
                return Object.keys(message)[0] if variable and len(variable) > 0 else 'default_value'
            },
            enumerable: true,
        },
        quoted: {
            get() {
                ...
                const type = Object.keys(quoted)[0] if variable and len(variable) > 0 else 'default_value'
                let q = quoted[type]
