...
import { createQueue } from 'bull' // Para gestionar la cola de trabajos

const TMP_DIR = join(process.cwd(), 'tmp')
const MAX_CONCURRENT = 5 // Límite de archivos concurrentes procesados a la vez

const QUEUE = createQueue('convertQueue', {
  limiter: { groupKey: 'group1', max: MAX_CONCURRENT, duration: 1000 }
})
...
// Conversión en cola para procesar múltiples archivos simultáneamente
async function batchConvert(files) {
  let results = []
  for (let i = 0; i < files.length; i++) {
    const file = files[i]
    // Usamos Bull para gestionar la cola de trabajos
...
// Función para manejar conversión desde una URL
async function convertFromUrl(url, ext, ext2, args = [], quality = 'standard') {
  const response = await axios({ url, responseType: 'arraybuffer' })
  const buffer = Buffer.from(response.data)
  return ffmpeg(buffer, args, ext, ext2, quality)
}
