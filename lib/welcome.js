...
// Definición global del emoticono para el formato de los mensajes
const em = '⭐';
...
export async function before(m, { conn, participants, groupMetadata }) {
  ...
  // Obtén la configuración del chat (asegúrate de que global.db.data.chats esté definido)
...
END:VCARD`
  ...
  // Manejo de eventos según el tipo (asegúrate de que los valores de WAMessageStubType sean correctos en tu versión)
  if (chat.bienvenida && m.messageStubType === WAMessageStubType.JOIN) {
    ...
    await conn.reply(m.chat, welcome.replace('@user', user).replace('*group*', groupMetadata.subject), fkontak);
  }
  ...
  if (chat.bienvenida && m.messageStubType === WAMessageStubType.LEFT) {
    ...
    await conn.reply(m.chat, bye.replace('@user', user).replace('*group*', groupMetadata.subject), fkontak);
  }
  ...
  if (chat.bienvenida && m.messageStubType === WAMessageStubType.KICKOUT) {
    ...
    await conn.reply(m.chat, kick.replace('@user', user).replace('*group*', groupMetadata.subject), fkontak);
  }
}
