...
// Define la variable global "em" antes de usarla
const em = '⭐';

export async function before(m, { conn, participants, groupMetadata }) {
  // Si no es un mensaje de grupo o no tiene un stub, no hace nada
  if (!m.messageStubType || !m.isGroup) return !0;

  // Define encabezado y pie de mensaje usando "em"
  let top = `*╭─${em}─── ⫍📢⫎ ───${em}─╮*\n`;
  let bottom = `\n*╰─${em}─── ⫍📢⫎ ───${em}─╯*`;
  
  // Asegúrate de que global.db.data.chats esté definido
  let chat = global.db.data.chats[m.chat];

  // Estructura para el mensaje de contacto
  let fkontak = {
    key: {
...
END:VCARD`
  ...
  // Manejo de bienvenida (si m.messageStubType es 27, por ejemplo)
  ...
  // Manejo de salida (si m.messageStubType es 28)
  ...
  // Manejo de expulsión (si m.messageStubType es 32)
