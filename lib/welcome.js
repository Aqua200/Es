...
// Define el emoticono global (puedes cambiarlo por el que prefieras)
const em = '⭐';

export async function before(m, { conn, participants, groupMetadata }) {
  if (!m.messageStubType || !m.isGroup) return !0;

  // Define los encabezados y pies de mensaje usando el emoticono
  let top = `*╭─${em}─── ⫍📢⫎ ───${em}─╮*\n`;
  let bottom = `\n*╰─${em}─── ⫍📢⫎ ───${em}─╯*`;
  
  // Asegúrate de que global.db.data.chats esté definido y tenga la propiedad 'bienvenida'
  let chat = global.db.data.chats[m.chat];
  
  // Estructura para el mensaje de contacto (utilizado como "fkontak")
  let fkontak = {
    key: {
...
END:VCARD`
  ...
  // 20 mensajes de despedida (cuando el miembro abandona voluntariamente)
  const leaveMessages = [
  ...
  // 20 mensajes de expulsión (cuando el miembro es removido por un admin)
  const kickMessages = [
  ...
  // Manejo de bienvenida (en el ejemplo original se usa m.messageStubType == 27)
  if (chat.bienvenida && m.messageStubType === 27) {
    let user = `@${m.messageStubParameters[0].split('@')[0]}`;
    let welcome = welcomeMessages[Math.floor(Math.random() * welcomeMessages.length)];
    await conn.reply(m.chat, top + welcome.replace('@user', user).replace('*group*', groupMetadata.subject) + bottom, fkontak);
  }

  // Manejo de salida (en el ejemplo original se usa m.messageStubType == 28)
  if (chat.bienvenida && m.messageStubType === 28) {
    let user = `@${m.messageStubParameters[0].split('@')[0]}`;
    let bye = leaveMessages[Math.floor(Math.random() * leaveMessages.length)];
    await conn.reply(m.chat, top + bye.replace('@user', user).replace('*group*', groupMetadata.subject) + bottom, fkontak);
  }

  // Manejo de expulsión (en el ejemplo original se usa m.messageStubType == 32)
  if (chat.bienvenida && m.messageStubType === 32) {
    let user = `@${m.messageStubParameters[0].split('@')[0]}`;
    let kick = kickMessages[Math.floor(Math.random() * kickMessages.length)];
    await conn.reply(m.chat, top + kick.replace('@user', user).replace('*group*', groupMetadata.subject) + bottom, fkontak);
  }
}
